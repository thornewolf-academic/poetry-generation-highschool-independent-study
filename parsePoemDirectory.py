...
def concat_dir_poems(inDirectory):
	'''
	This function takes an input directory and returns the concated string containing every text file in the entire directory
	The line !='style' is because in each poem type there should be a description detailing the style each poem has so future generation will know what to do.
	'''
	_returnString=''
...
def gen_freq_dict(stringArg):
	'''
	This takes an input string an makes a dictionary to show what words follow each unique word. 
	Also generates a list of words that are found ad the start of a line/sentence.
	'''
	stringArg=stringArg.replace('\n','. ')
	_returnDict={'START':[],'END':[]}
	listOfWords=stringArg.split()
	for eachIndex in range(len(listOfWords)-1):

		key=listOfWords[eachIndex] #This is the word that will have a list assigned to it.
		key=key.replace(',','')
		key=key.replace('-','')
		value=listOfWords[eachIndex+1].replace('.','') #The first word that follows the word in interest.
		if eachIndex>0:
			if listOfWords[eachIndex-1][-1] in '.!?':
				if key!='.':
					_returnDict['START'].append(key.strip(".:;?!’'").replace(u"\u2018", "'").replace(u"\u2019", "'"))
		if key[-1] not in '.!?':
			if key not in _returnDict: #If this word has not occured so far in the data analysis. listOfWords[eachIndex]='word' ?
				_returnDict[key.strip(".:;?!'’")]=[value.strip(".:;?!'’")]
			else:
				#TODO
				#This will add to the list value that coresponds to the dictionary key in the event that the key already exists	
				_returnDict[key].append(value.replace(u"\u2018", "'").replace(u"\u2019", "'"))
...
'''
myDict=gen_freq_dict(test_string)

for key in sorted(list(myDict.keys())):
	print(key,random.sample(myDict[key],1),len(myDict[key]))
print(myDict['START'])
'''
